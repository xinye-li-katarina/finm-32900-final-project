...
class TestFormattedTable(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # **修正 cwd 确保和 Notebook 一致**
        script_dir = os.path.dirname(os.path.abspath(__file__))  # `src/`
        project_root = os.path.abspath(os.path.join(script_dir, ".."))  # `finm-32900-final-project`
        os.chdir(project_root)
        print(f"✅ New Working Directory: {os.getcwd()}")

        # **手动修正 Table02Prep.py 访问的路径**
        table02prep_path = os.path.join(project_root, "src", "Table02Prep.py")

        with open(table02prep_path, "r") as file:
            content = file.read()

        if "../data/manual/ticks.csv" in content:
            new_content = content.replace("../data/manual/ticks.csv", "data/manual/ticks.csv")
            
            with open(table02prep_path, "w") as file:
                file.write(new_content)

            print("✅ Updated Table02Prep.py to use correct CSV path!")
